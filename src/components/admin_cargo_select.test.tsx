...
describe('AdminCargoSelect', () => {
  it('will render', async () => {
    ...
    await waitFor(() => expect(qt.queryAllByText('Loading Test').length).toBe(0))
    await waitFor(() => expect(qt.queryAllByText('Flare Hazard Placards (Note 1)').length).toBe(1))
  })
  ...
  it('will be null if !ep.includes(configCargo)', async () => {
    ...
    const qt = renderWrapped(<AdminCargoSelect validate={fn}/>)
    await waitFor(() => expect(qt.queryAllByText('Loading Test').length).toBe(0))
    await waitFor(() => expect(qt.queryAllByText('Flare Hazard Placards (Note 1)').length).toBe(0))
  })
  ...
  it('will be show error if no cargos are able to be selected', async () => {
    ...
    const qt = renderWrapped(<AdminCargoSelect validate={fn}/>)
    await waitFor(() => expect(qt.queryAllByText('Loading Test').length).toBe(0))
    await waitFor(() => expect(qt.queryAllByText('Please add cargo to insert into config').length).toBe(1))
  })
  ...
  it('will change the change the cargo inside of a config', async () => {
    ...
    await waitFor(() => expect(ct.queryAllByText('Flare Hazard Placards (Note 1)').length).toBe(1))
    expect(fn.mock.calls.length).toBe(0)

    fireEvent.mouseDown(ct.getByText('Flare Hazard Placards (Note 1)'))
    await waitFor(() => expect(ct.queryAllByText('Water Container (5 Gallon)').length).toBe(1))
