...
export const JsonTable = () => {
  ...
  const table = useMemo(() => {
    ...
    { title: 'Delete / Edit',
      ...
      <Row>
        <Col span={6}>
          <DeleteOutlined style={{ fontSize: '32px'}} onClick={() => alert(JSON.stringify(row))}/>
        </Col>
        <Col span={6} offset={6}>
          <EditOutlined style={{ fontSize: '32px'}} onClick={() => alert(JSON.stringify(row))}/>
        </Col>
      </Row>
    ...
    const filteredData = (data as Record<string, unknown>[]).map(
      row => Object.fromEntries(filteredKeys.map(k => ([k,row[k]])))
    )
