...
export const ConfigSelect = () => {
  ...
  return (
   <Select onChange={onChange} defaultValue={0} style={{width: Const.SELECT_WIDTH}}>
     <Option value={0}>No Config</Option>
     {air.configs.map(c => <Option value={c.configId} key={c.configId}>{c.name}</Option>)}
   </Select>
  )
}
