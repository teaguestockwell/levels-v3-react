...
describe('TankRow', () => {
  it('will render', async () => {
    const {getByText, queryAllByText} = renderWrapped(<TankRow tank={tank} cargoString={cargoString}/>)
  ...
  it('will change tank weight and fs', async () => {
    ...
    await waitFor(() => {
      expect(CargoStore.getState().cargoMap.get('0')).toStrictEqual({...cargoString, weightEA: '500', fs: "1120"})
    })
  })
})