...
import { cargoToNewCustomCargo, getNewCustomCargoString, getYupSchema } from "../util"

export const UserCargo = () => {
  const [putCargo, putCargoIsValid] = CargoStore(state => [state.putCargo, state.putCargoIsValid])
  ...
  const menu = (
    <Menu onClick={onAddAddACargoClick}>
      <Menu.SubMenu title="Steward">
        {stewardCargo.map(c => <Menu.Item key={c.cargoId}>{c.name}</Menu.Item>)}
      </Menu.SubMenu>
      <Menu.SubMenu title="Emergency">
        {emergencyCargo.map(c => <Menu.Item key={c.cargoId}>{c.name}</Menu.Item>)}
      </Menu.SubMenu>
      <Menu.SubMenu title="Extra">
        {extraCargo.map(c => <Menu.Item key={c.cargoId}>{c.name}</Menu.Item>)}
      </Menu.SubMenu>
    </Menu>
  );
  ...
  return <>
    ...
    <Dropdown overlay={menu} trigger={['click']}>
      <Button>Add AddA<DownOutlined /></Button>
    </Dropdown>
