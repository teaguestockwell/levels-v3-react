...
export function CargoForm(props: ICargo) {
  ...
  const aircraft = AircraftStore(state => state.selectedAircraft)
  ...
  function validateForm(values: ICargoString) {
    ...
    else if(isNaN(parseFloat(values.weight))){errors.weight = 'Must be a number'}
    else if(parseFloat(values.weight) <= 0){errors.weight = 'Must be > 0'}
    // @ts-ignore
    else{if(parseFloat(values.weight) > aircraft.cargoweight1){errors.weight = `Must be <= ${aircraft.cargoweight1}`}}

    // fs
    if (!values.fs) {errors.fs = 'Required'}
    else if(isNaN(parseFloat(values.fs))){errors.fs = 'Must be a number'}
    // @ts-ignore
    else if(parseFloat(values.fs) < aircraft.fs0){errors.fs = `Must be >= ${aircraft.fs0}`}
    // @ts-ignore
    else{if(parseFloat(values.fs) > aircraft.fs1){errors.fs = `Must be <= ${aircraft.fs1}`}}
 
    // qty
    if (!values.qty) {errors.qty = 'Required'}
    else if(isNaN(parseInt(values.qty))){errors.qty = 'Must be a number'}
    else if(isInteger(parseInt(values.qty))){errors.qty = 'Must be an integer'}
    else{if((parseInt(values.qty) <= 0)){errors.qty = 'Must be > 0'}}
      ...
      >
        {({submitForm, isSubmitting, touched, errors, validateForm}) => (
          <Form>
            {getFields()}
            <Box margin={1}>
              <Button variant="contained" color="primary" onClick={()=> deleteCargo(props.id)}>
                Delete
              </Button>
            </Box>
          </Form>
